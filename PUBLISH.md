...
**公開済みです。**（リポジトリは public / v1.0.0 を 2026-08-23 に Release）
下の項目には**まだ済んでいないもの**が混じっています。チェックの付いていないものが残作業です。
...
| `/dictionary.txt`（リポジトリ直下） | **.gitignore 済み** | 旧形式の個人辞書。移動せず利用を継続する |

作業:

- [x] いまの `dictionary.txt` から、一般的な語彙だけを抜いて `dictionary.sample.txt` を作った
- [x] 既存の個人辞書・シンボリックリンクは移動も上書きもしない
- [x] `build.sh` は公開サンプルだけを同梱する。旧辞書はローカルで引き継ぐ
- [x] 設置前に辞書をローカルへバックアップし、失敗したら旧アプリを残す
- [x] `.claude/skills/dictionary/` の説明を個人辞書の保護と合わせる
...
個人辞書は公開リポジトリで共有しません。自動同期も行いません。必要なら会社の規則を含め、利用者が別途方法を選びます。
...
- [x] **Releases に .app を置くかどうか。**
      署名と公証（Notarization）が無いと Gatekeeper に止められ、
      「開発元を確認できません」で終わります。Apple Developer Program（年間費用）が要る話なので、
      当面は**ソースからビルドしてもらう**方針。README の「ソースからビルドする前に」に理由を書いた。

      **この方針の代償を、README の頭で先に払っておくこと。**
      ソースからビルドする以上、利用者は**自己署名証明書を自分で作る必要があります**
      （作らないとアドホック署名になり、入力監視が黙って拒否される）。
      これを「開発するとき」の項に置いていたせいで、実際に別の Mac で丸一日溶かしました。
      いまは導入手順そのものの前に置いてあります。順番を戻さないこと。
      配布版を出す日が来たら「配布版なら証明書は不要／許可はそれでも必要」の区別も
      README に書いてあるので、そのまま使えます。
- [ ] Issues / Pull Request の受け方（方針は CLAUDE.md の「運用」にある）
- [ ] v1.0.0 を付けるのはいつか。「1週間使って直したいところが出なくなったら」が目安
- [x] `AGENTS.md` / `.agents/skills/` は Codex 向けの写しとして出す。正本との差分がない状態にする
