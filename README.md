...
追跡ファイルに変更がある場合や辞書を読めない場合は中止します。勝手な reset や stash はしません。
...
目印は、フォーカス中の入力欄がある画面へ追従します。画面内の位置は従来どおり、
...
- リポジトリ直下の `dictionary.txt` は移動せず、そのファイルを読み続けます。
  個人辞書がまだ無ければそこへのリンクを作り、別の個人辞書があれば両方を読みます。
...
退避に失敗したらアプリを入れ替えません。バックアップは Git の外にあり、自動削除しません。
