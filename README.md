...
*実際の速度です。早送りしていません。* — [全編 26 秒・音あり](docs/nobetsu-demo.mp4)
...
**⌘ を押している間に他のキーやクリック、ホイールが来たら、
それはショートカットだと見なして発火しません。**
⌘Tab や ⌘S を打とうとしてためらっても、⌘＋ホイールで拡大縮小しても暴発しません。
...
- **右の ⌘ だけで開始する**
- **入力先から離れたら止める** — 既定で有効。下の説明を読んでください
- **遠距離マイク補正** — マイクから離れて話すとき
- **辞書を編集する / 読み直す** — 下の「辞書」を読んでください
...
同じアプリの中で別の入力欄へ移っただけなら止まりません。
設定メニューを開いている間も止まりません。

**自分で止めたときとは違う音が鳴ります。** 押していないのに終わるので、
同じ音だと何が起きたのか分かりません。止まった理由はメニューにも残ります。

一日中つけっぱなしにして、あちこちで喋りたい場合はメニューから切ってください。

### 辞書

音声認識は「Clone」を「苦労」と書きます。話し言葉としては正しいので、
認識器の側では直りません。**打ち込む直前に、こちらで書き換えます。**

```
認識結果 => 打ちたい文字
```

という1行1組のただのテキストです。辞書は2枚あります。

| | 置き場所 | 誰のものか | 反映のしかた |
|---|---|---|---|
| 共有辞書 | リポジトリの `dictionary.txt` | git で共有される | `./build.sh` |
| 個人辞書 | `~/Library/Application Support/nobetsu/dictionary.txt` | その Mac だけ | 保存するだけ |

同じ言葉が両方にあれば個人辞書が勝ちます。
...
**短い言葉を左に置かないでください。** 「苦労」を `Clone` にすると、
本当に苦労した話が書けなくなります。少し長めに取るのが安全です。
長い言い回しから先に当てるので、「クローン」と「クローンしてくる」を
両方書いても、長い方が勝ちます。

複数の Mac で使う場合、両方で効かせたい言い換えはリポジトリ側に書いて push、
その Mac だけの事情は個人辞書へ、と分けてください。

自分でリポジトリを持っていて、**共有辞書をビルドせずに効かせたい**場合は、
個人辞書をリポジトリの辞書へのリンクにしておく手があります。

```bash
ln -s "$(pwd)/dictionary.txt" ~/Library/Application\ Support/nobetsu/dictionary.txt
```

こうすると、`dictionary.txt` を書き換えた時点で効きます（追加のみ。
規則を消したときはアプリに焼き込まれた古い辞書が残るので `./build.sh` が要ります）。

#### いつ、どう当たっているのか

**AI は関わりません。**アプリが自分でファイルを読み、文字列を置き換えているだけです。
辞書がどこかへ送られることも、ネットワークに出ることもありません。

読むのは**喋りはじめる瞬間の1回だけ**です。2枚のファイルの更新時刻を見て、
変わっていなければ読み直しません。認識中は一切読みません。

```
⌘ 長押し
  ↓
辞書を読む ── # から行末を捨てる → `=>` で割る → 文字列の組の配列にする
  │            同梱辞書 → 個人辞書 の順に重ねる（同じ左辺なら個人辞書が勝つ）
  │            左が右の一部になる規則を捨てる（自分の結果を直しにいくため）
  │            左辺の長い順に並べ替える（「苦労」より「苦労してくる」を先に当てる）
  ↓
マイク開始
```

**説明文はここで消えます。**辞書に何行コメントを書いても、実行時には
文字列の組しか残りません。速度にも影響しません。

喋っている間、認識器は「**その区間の全文**」を毎秒4回ほど送ってきます。
届くたびに、全部の規則を順に当てます。

```
DictationTranscriber
  ↓  「苦労してくるね」（区間の全文）
辞書を当てる ── 規則の数だけ replacingOccurrences するだけ。状態は持たない
  ↓  「Cloneしてくるね」
...
前回何を置き換えたかを覚えていないのが大事なところです。毎回その区間の全文が届くので、
覚える必要がありません。おかげで、認識が「くろー」→「クローン」→「Cloneしてくる」と
変わっていく途中で一瞬おかしな形になっても、**次の更新で正しく打ち直されます。**
