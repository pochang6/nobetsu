...
同梱辞書のほうは見本です。自分用に育てたものを焼き込みたい場合は、リポジトリ直下に
`dictionary.txt` を置くとそちらが優先されます。**このファイルは `.gitignore` してあります。**
辞書には仕事の固有名詞や、そのとき考えていたことが溜まっていくためです。
...
副作用を持たない部分——**辞書の置換**と**打ち込みの差分計算**——を確かめます。
アプリを起動せずに走ります（数秒）。

この2つに絞っているのは、**間違えると利用者の書きかけの文章を壊す**場所であり、
かつ入力と出力だけで完結していて機械で確かめられるからです。
